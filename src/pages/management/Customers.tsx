    ...
    transactions: [
      {
        ...
        type: "Order",
        amount: 5000,
        description: "Summer Collection Print",
        status: "Completed",
      },
      {
        id: "T2",
      ...
      },
      {
        ...
        type: "Order",
        amount: 17500,
        description: "Branding Material Print",
        status: "Completed",
      },
    ],
    materials: [
    ...
    transactions: [
      {
        ...
        type: "Order",
        amount: 1200,
        description: "One-Time Custom Hoodie",
        status: "Completed",
      },
      {
        id: "T5",
...
const CustomerAvatar = ({
  ...
  if (avatar) {
    return (
      <div
        className={cn(
          "rounded-[22px] overflow-hidden border-4 border-white shadow-xl",
          sizeClasses[size],
        )}
      >
...
const Customers = () => {
  const isBackendMode = true;
  const [customers, setCustomers] = useState<Customer[]>(() =>
    isBackendMode ? [] : initialCustomers,
  );
  ...
  const totalDues = customers.reduce(
    (acc, c) => acc + (c.balance < 0 ? Math.abs(c.balance) : 0),
    0,
  );
  ...
  return (
    <ManagementLayout>
      <div className="space-y-8 animate-fade-in-up">
        ...
        <div className="flex flex-col lg:flex-row gap-6 justify-between items-start lg:items-center">
          <div className="space-y-1">
            ...
            <p className="text-slate-400 text-sm font-medium">
              Manage accounts, materials, and cloud storage for{" "}
              <strong>{customers.length}</strong> client
            ...
            {loadError && (
              <div className="mt-2 p-4 rounded-2xl bg-red-50 border border-red-200 flex flex-col gap-2">
                ...
                <p className="text-slate-500 text-[10px]">
                  Ensure the backend is running and you are logged in as admin.
                  Then click Refresh.
                </p>
              </div>
            )}
        ...
        {/* Global Statistics Cards — real data from API / list */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-white p-6 rounded-[32px] border border-slate-100 shadow-sm flex items-center gap-5">
            ...
            <div>
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-1">
                {isBackendMode ? "Total Orders" : "Outstanding Dues"}
              </p>
              <p className="text-2xl font-black text-red-600">
                {isBackendMode
                  ? totalOrdersSum.toLocaleString()
                  : `${totalDues.toLocaleString()} EGP`}
              </p>
            </div>
          </div>
          <div className="bg-white p-6 rounded-[32px] border border-slate-100 shadow-sm flex items-center gap-5">
            ...
            <div>
              ...
              <p className="text-2xl font-black text-slate-900">{activeCustomersCount}</p>
            </div>
          </div>
        </div>
          ...
          {loading ? (
            ...
            <div className="col-span-full py-16 text-center rounded-[40px] border-2 border-dashed border-slate-200 bg-slate-50/50">
              ...
              <p className="text-slate-400 text-xs mt-2 max-w-md mx-auto">
                Customers appear here when they register via the app. If you
                expect to see accounts, click Refresh or check the backend.
              </p>
              <button
                type="button"
                onClick={refetch}
                className="mt-4 px-6 py-2 rounded-xl bg-primary text-white text-xs font-black uppercase"
              >
                Refresh
              </button>
            </div>
            ...
            filteredCustomers.map((customer) => (
              ...
              >
                ...
                <div className="space-y-2 mb-8 relative z-10">
                  <h3 className="text-2xl font-black text-slate-900 tracking-tight group-hover:text-primary transition-colors uppercase">
                    {customer.name}
                  </h3>
                ...
                <div className="grid grid-cols-2 gap-6 mb-8 relative z-10">
                  <div className="space-y-1">
                    ...
                    >
                      {customer.balance === 0
                        ? "SETTLED"
                        : `${Math.abs(customer.balance).toLocaleString()} EGP`}
                    </p>
                  </div>
        ...
        <SheetContent className="w-full sm:max-w-xl p-0 overflow-hidden border-none flex flex-col">
          <div className="bg-primary p-12 text-white relative">
            ...
            <SheetHeader>
              ...
              <SheetDescription className="text-white/60 font-medium text-sm">
                Create a fully integrated factory account for your customer.
              </SheetDescription>
            </SheetHeader>
          </div>
          ...
          {isBackendMode && (
            <div className="mx-6 mt-4 p-4 rounded-2xl bg-primary/10 border border-primary/20 text-primary text-xs font-bold uppercase tracking-wide">
              Customers are added when they register via the app. Use Refresh to
              see new registrations.
            </div>
          )}
          <form
            onSubmit={handleAddCustomer}
            className="flex-1 overflow-y-auto p-12 space-y-8 bg-white"
          >
            {/* Account Identity */}
            ...
            <div className="space-y-6 pt-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <button
                  type="button"
                  onClick={() =>
                    setNewCustomer({ ...newCustomer, type: "Regular" })
                  }
                  className={cn(
                ...
                >
                  <p className="font-black text-primary text-sm uppercase mb-1">
                    Regular
                  </p>
                  <p className="text-[10px] text-slate-400 font-bold uppercase leading-relaxed">
                    Recurrent Client. Supports Dues & Payments.
                  </p>
                </button>
                <button
                  type="button"
                  onClick={() =>
                    setNewCustomer({ ...newCustomer, type: "One-Time" })
                  }
                  className={cn(
                    "p-6 rounded-3xl border-2 text-left transition-all",
                    newCustomer.type === "One-Time"
                      ? "border-accent bg-accent/5 ring-4 ring-accent/5"
                      : "border-slate-100 opacity-60 grayscale hover:grayscale-0 hover:opacity-100",
                  )}
                >
                  <p className="font-black text-accent text-sm uppercase mb-1">
                    One-Time
                  </p>
                  <p className="text-[10px] text-slate-400 font-bold uppercase leading-relaxed">
                    Single Project. Pre-paid cycle only.
                  </p>
                </button>
              </div>
            </div>
              ...
              <div className="flex-1 overflow-y-auto px-12 py-8">
                <Tabs defaultValue="overview" className="space-y-8">
                  <TabsList className="bg-transparent border-b border-slate-200 w-full justify-start gap-8 rounded-none h-auto p-0">
                    <TabsTrigger
                      value="overview"
                      className="data-[state=active]:bg-transparent data-[state=active]:border-primary data-[state=active]:text-primary border-b-4 border-transparent px-2 py-4 rounded-none h-auto text-[11px] font-black uppercase tracking-widest text-slate-400"
                    >
                      Overview
                    </TabsTrigger>
                    <TabsTrigger
                  ...
                  >
                    <div className="bg-white rounded-[32px] border border-slate-100 overflow-hidden shadow-sm">
                      <div className="p-6 border-b border-slate-50 flex justify-between items-center bg-slate-50/50">
                        <h4 className="font-heading font-black text-xl uppercase tracking-tight">
                          Ledger History
                        </h4>
                        ...
                        <table className="w-full text-left">
                          <thead>
                            <tr className="bg-slate-50">
                              <th className="px-8 py-4 text-[9px] font-black text-slate-400 uppercase">
                                Ref ID
                              </th>
                              <th className="px-8 py-4 text-[9px] font-black text-slate-400 uppercase">
                                Date
                      ...
                      {selectedCustomer.materials.map((m) => (
                        ...
                        >
                          ...
                          <div className="flex items-center justify-between pt-4 border-t border-slate-50 text-[10px] font-bold text-slate-400">
                            ...
                            <button className="text-primary underline underline-offset-4 uppercase">
                              View History
                            </button>
                          </div>
                        </div>
                      ...
                      <div className="flex justify-between items-center">
                        <div>
                          ...
                          <p className="text-slate-400 text-xs font-medium mt-1">
                            Manage client-side web portal login details.
                          </p>
                        </div>
                        <div className="flex items-center gap-3">
                    ...
                    <div className="bg-red-50/50 p-10 rounded-[32px] border border-red-100 space-y-6">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-red-700/60 text-xs font-medium">
                            Suspending an account will disable their ability to
                            place orders on the web-portal.
                          </p>
                        </div>
                      </div>
